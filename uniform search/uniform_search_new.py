...
def depth_first_search(maze,start,goal):
	...
	while heap:
		...
		if x==goal[0] and y==goal[1]:
			...
			name = 'Uniform First Search\n'
			#goal_node = maze[x][y][0]
			return print(name,'Goal Node =',goal_node,'Start Node =',start_node,'Cost',cost,'Time =',time,'\n','Visited Nodes \n ',visited_node,'\n Final Path \n',path)
			#break
		...
		#Calculate the coordinates of the neighbor
			...
			if 0 <= nx < len(maze) and 0 <= ny < len(maze) and maze[nx][ny][1] != 'B' and maze[nx][ny][0] not in visited_node:
				new_node=(nx,ny)
				heapq.heappush(heap,(cost+1,new_node, path + [maze[nx][ny][0]]))
