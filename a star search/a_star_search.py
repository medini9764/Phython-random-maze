...
def a_star_search(maze,start,goal):
    ...
    #Create a queue to store the cells that need to be explored
    ...
    #Create list to store the sum of heuristic values and cost of edge
    sum_of_cost=[heuristic_start_cost]
    ...
    while heap:
        ...
        #Calculate the coordinates of the neighbor
            ...
            if 0 <= nx < len(maze) and 0 <= ny < len(maze) and maze[nx][ny][1] != 'B' and maze[nx][ny][0] not in visited_node:
                ...
                cost = abs(nx- goal[1])+abs(ny-goal[0])
                heuristic_cost.append(cost)
                cost=cost+1
                sum_of_cost.append(cost)
                new_node=(nx,ny)
                heapq.heappush(heap,(cost,new_node, path + [maze[nx][ny][0]]))
                visited_node.append(maze[nx][ny][0])
                time +=1

    return print('Goal Node =',goal_node,'Start Node =',start_node,'Cannot find a path using A Star Search')
