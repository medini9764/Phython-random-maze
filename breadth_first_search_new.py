...
#Randomly select six barrier nodes from the remaining 62 nodes in the maze.
barrier_nodes = []
for i in range(6):
	while True:
...
def depth_first_search(maze,start,goal):
	...
	# staring node and [start] as the path
	queue = [(start,[maze[start[0]][start[1]][0]])]
	...
	while queue:
		...
		#Calculate the coordinates of the neighbor
			...
			if 0 <= nx < len(maze) and 0 <= ny < len(maze) and maze[nx][ny][1] != 'B' and maze[nx][ny][0] not in visited_node:
				new_node=(nx,ny)
				queue.append((new_node,path + [maze[nx][ny][0]]))
